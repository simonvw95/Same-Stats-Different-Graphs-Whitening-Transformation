...
line_shapes = ['x', 'h_lines', 'v_lines', 'wide_lines', 'high_lines', 'slant_up', 'slant_down', 'center', 'star',
               'down_parab']
all_targets = list(line_shapes)
all_targets.extend(['circle', 'bullseye', 'dots'])
initial_datasets = ['dino', 'random_cloud', 'slant', 'big_slant']
max_time = 1800
#
...
def load_dataset(name="dino"):
    if name == "dino":
        df = pd.read_csv(f"seed_datasets/Datasaurus_data.csv", header=None, names=['x', 'y'])
    elif "random_cloud" in name:
        df = pd.read_csv("seed_datasets/{}.csv".format(name))
    elif name == "slant":
        df = pd.read_csv(f"seed_datasets/slanted_less.csv")
        df = df[['x', 'y']]
    elif name == "big_slant":
        df = pd.read_csv(f"seed_datasets/less_angled_blob.csv")
        df = df[['x', 'y']]
        df = df.clip(1, 99)
...
def save_scatter(df, iter, directory: str, dp=72):
    show_scatter(df)
    plt.savefig(f"{directory}/{str(iter)}.png", dpi=dp)
    plt.clf()
    plt.cla()
    plt.close()


def save_scatter_and_results(df, iter, directory: str, dp=72, labels=["X Mean", "Y Mean", "X SD", "Y SD", "Corr."]):
...
def show_scatter(df, xlim=(-25, 125), ylim=(-25, 125), color="black", marker="o", reg_fit=False):
...
def show_scatter_and_results(df, labels=["X Mean", "Y Mean", "X SD", "Y SD", "Corr."]):
    ...
    plt.text(130, y_off + 80, labels[0].ljust(max_label_length) + ": " + format(res[0], "0.9f")[:-2], fontsize=fs,
             alpha=0.3)
    plt.text(130, y_off + 65, labels[1].ljust(max_label_length) + ": " + format(res[1], "0.9f")[:-2], fontsize=fs,
             alpha=0.3)
    plt.text(130, y_off + 50, labels[2].ljust(max_label_length) + ": " + format(res[2], "0.9f")[:-2], fontsize=fs,
             alpha=0.3)
    plt.text(130, y_off + 35, labels[3].ljust(max_label_length) + ": " + format(res[3], "0.9f")[:-2], fontsize=fs,
             alpha=0.3)
    plt.text(130, y_off + 20, labels[4].ljust(max_label_length) + ": " + format(res[4], "+.9f")[:-2], fontsize=fs,
             alpha=0.3)

    plt.text(130, y_off + 80, labels[0].ljust(max_label_length) + ": " + format(res[0], ".9f")[:-7], fontsize=fs,
             alpha=1)
    plt.text(130, y_off + 65, labels[1].ljust(max_label_length) + ": " + format(res[1], "0.9f")[:-7], fontsize=fs,
             alpha=1)
    plt.text(130, y_off + 50, labels[2].ljust(max_label_length) + ": " + format(res[2], "0.9f")[:-7], fontsize=fs,
             alpha=1)
    plt.text(130, y_off + 35, labels[3].ljust(max_label_length) + ": " + format(res[3], "0.9f")[:-7], fontsize=fs,
             alpha=1)
    plt.text(130, y_off + 20, labels[4].ljust(max_label_length) + ": " + format(res[4], "+.9f")[:-7], fontsize=fs,
             alpha=1)
...
def fitness(df1, df2):
    ...
    pos_1 = scale(df1.to_numpy())
    pos_2 = scale(df2.to_numpy())

    pdist = distance_matrix(pos_1, pos_2)

    return pdist.min(axis=0).mean()
...
#
def perturb(df, initial, target_df, target='circle',
            line_error=1.5,
            shake=0.1,
            allowed_dist=2,
            temp=0,
    ...
    while True:
        ...
        # old_fit = fitness(df[['x', 'y']], target_df)
        # new_df = df.copy()
        # new_df.loc[row, 'x'] = xm
        # new_df.loc[row, 'y'] = ym
        # new_fit = fitness(new_df[['x', 'y']], target_df)
        ...
        if 'circle' in target:
            ...
            old_dist = np.min([dist([x, y], [df['x'][row], df['y'][row]]) for x, y in target_df[['x', 'y']].to_numpy()])
            new_dist = np.min([dist([x, y], [xm, ym]) for x, y in target_df[['x', 'y']].to_numpy()])
        ...
        if new_dist < old_dist or new_dist < allowed_dist or do_bad:  # and
            ...
            break
        # if new_fit < old_fit or new_fit < allowed_dist or do_bad:  # and
        #     # ym > y_bounds[0] and ym < y_bounds[1] and xm > x_bounds[0] and xm < x_bounds[1]):
        #     break
...
#
def run_pattern(df, target, directory, target_df, iters=100000, num_frames=100, decimals=2, shake=0.2,
    ...
    for i in looper(iters + 1, leave=True, ascii=True, desc=target + " pattern"):
        ...
        test_good = perturb(r_good.copy(), initial=df, target=target, temp=t, target_df=target_df)
        ...
        for x in range(write_frames.count(i)):
            ...
            r_good.to_csv(f'{directory}/{target}' + "-data-" + format(int(frame_count), '05') + ".csv")
...
def do_single_run(start_dataset, target, target_df, iterations=100000, decimals=2, num_frames=100):
    global it_count
    global frame_count
    it_count = 0
    frame_count = 0
    ...
    temp = run_pattern(df, target, target_df=target_df, iters=iterations, num_frames=num_frames, directory=f'datasaurus_dozen/results/{start_dataset}_{target}')
...
def create_gifs(spec_directory, shape_start, shape_end):
    # Create the frames
    frames = []
    imgs = glob.glob("{}/results/{}_{}/*.png".format(spec_directory, shape_start, shape_end))
    for i in imgs:
        # exclude projection step saved by sgd_implem.py
        if 'project' not in i:
            new_frame = Image.open(i)
            frames.append(new_frame)

    # Save into a GIF file that loops forever
    if not path.exists('{}/results/gifs'.format(spec_directory)):
        os.mkdir('{}/results/gifs'.format(spec_directory))

    frames[0].save("{}/results/gifs/{}_{}.gif".format(spec_directory, shape_start, shape_end), format='GIF',
...
if __name__ == '__main__':
    ...
    shape_ends = ['datasaurus', 'x', 'h_lines', 'v_lines', 'wide_lines', 'high_lines', 'slant_up', 'slant_down', 'star',
                   'down_parab', 'circle', 'bullseye', 'dots']
    ...
    for shape_end in shape_ends:
        ...
        # shape_end = 'star'
        save_directory = f'datasaurus_dozen/results/{shape_start}_{shape_end}'
        ...
        target_df = pd.read_csv("target_datasets/{}_250.csv".format(shape_end), header=None, names=['x', 'y'])

        temp = do_single_run(shape_start, shape_end, target_df = target_df, iterations=it, decimals=de, num_frames=frames)

        save_pdf(df=temp[['x', 'y']], df_target=target_df, save_location=save_directory + '/' + shape_end, target_shape=shape_end + "_142", t_xsd=t_xsd, t_pc=t_pc,
                 method='DD')

        create_gifs(spec_directory = 'datasaurus_dozen', shape_start=shape_start, shape_end = shape_end)
